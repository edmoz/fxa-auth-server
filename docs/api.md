...
This endpoint may send a verification email to the user.  Callers may optionally provide the `rpid` parameter to indicate what Relying Party they are acting on behalf of.  This is an opaque hexadecimal token which will be embedded in the verification link as a query parameter.
...
* rpid - opaque hexadecimal token to be included in verification links
